...
<table class="table table-bordered" id="resultTable" data-responsive="table">
	...
	<tbody>
			...
			<tr>
				...
				$sdsd=$_GET['invoice'];
				$resultas = $dbh->prepare("SELECT sum(amount) FROM sales_order WHERE invoice= :a");
				...
				<td colspan="1"><strong style="font-size: 12px; color: #222222;">
			<?php 
				$resulta = $dbh->prepare("SELECT sum(profit) FROM sales_order WHERE invoice= :b");
...
<a rel="facebox" href="checkout.php?pt=<?php echo $_GET['id']?>&invoice=<?php echo $_GET['invoice']?>&total=<?php echo $fgfg ?>&totalprof=<?php echo $asd ?>"><button class="btn btn-success btn-large btn-block"><i class="icon icon-save icon-large"></i> SAVE</button></a>
